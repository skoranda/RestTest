...
class OrgIdentityTest {
  public $apiUser;
  public $name = 'OrgIdentity';
  public $plural = 'OrgIdentities';
  public $controller = 'org_identities';
  ...
  public function run() {
    ...
    $body["$this->plural"][0]['Affiliation'] = 'staff';
    $body["$this->plural"][0]['Title'] = 'Created by REST API client';
    $body["$this->plural"][0]['O'] = 'None';
    $body["$this->plural"][0]['Ou'] = 'None';
    ...
    $body["$this->plural"][0]['Title'] = 'Edited by REST API client';
    ...
    if(!$foundIt) {
      $msg = "$this->name View (per CO) did not find created";
      throw new Exception($msg);
    }
    ...
    $request['uri']['path'] = "$this->controller.json";
    $request['uri']['query']['coid'] = 2;
    $request['uri']['search.identifier'] = 'skoranda@illinois.edu';

    try {
      $result = $client->call($request, 200);
    } catch (Exception $e) {
      $msg = "$this->name View (per Identifier) exception:" . $e->getMessage();
      throw new Exception($msg);
    }
