...
App::uses('OrgIdentity', 'Model');

class CoOrgIdentityLinkTest {
  public $apiUser;
  public $name = 'CoOrgIdentityLink';
  public $plural = 'CoOrgIdentityLinks';
  public $controller = 'co_org_identity_links';
  ...
  public function run() {
    ...
    // Create two OrgIdentity objects to use for testing.
    $orgIdentityModel = new OrgIdentity();

    $data = array();
    $data['OrgIdentity']['co_id'] = 2;
    $data['OrgIdentity']['affiliation'] = AffiliationEnum::Member;

    $orgIdentityModel->clear();
    $orgIdentityModel->save($data);
    $orgId1 = $orgIdentityModel->id;

    $orgIdentityModel->clear();
    $orgIdentityModel->save($data);
    $orgId2 = $orgIdentityModel->id;

    // Create a CoPerson object to use for testing.
    $coPersonModel = new CoPerson();

    $data = array();
    $data['CoPerson']['co_id'] = 2;
    $data['CoPerson']['status'] = StatusEnum::Active;

    $coPersonModel->clear();
    $coPersonModel->save($data);
    $coPersonId = $coPersonModel->id;
    ...
    $body["$this->plural"][0]['CoPersonId'] = $coPersonId;
    $body["$this->plural"][0]['OrgIdentityId'] = $orgId1;
    ...
    $body["$this->plural"][0]['OrgIdentityId'] = $orgId2;
    ...
    $request['method'] = 'GET';
    $request['uri']['path'] = "$this->controller.json";
    $request['uri']['query']['copersonid'] = $coPersonId;
    ...
    if(!$foundIt) {
      $msg = "$this->name View (per identity) did not find created";
      throw new Exception($msg);
    }
    ...
    // Delete the CoPerson object created for testing.
    $coPersonModel->delete($coPersonId);


    // Delete the two OrgIdentity objects created for testing.
    $orgIdentityModel->delete($orgId1);
    $orgIdentityModel->delete($orgId2);
  }
}
