...
class NavigationLinkTest {
  ...
  public $plural = 'NavigationLinks';
  public $controller = 'navigation_links';
  ...
  public function run() {
    ...
    $body["$this->plural"][0]['Title'] = 'Some Title';
    $body["$this->plural"][0]['Url'] = "https://some.org";
    $body["$this->plural"][0]['Location'] = 'topbar';
    ...
    $body["$this->plural"][0]['Url'] = 'https://some.other.org';
    ...
    $foundIt = false;
    foreach($response->{$this->plural} as $i) {
      if($i->Id == $newId) {
        $foundIt = true;
        $break;
      }
