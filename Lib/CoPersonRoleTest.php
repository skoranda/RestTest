...
class CoPersonRoleTest {
  public $apiUser;
  public $name = 'CoPersonRole';
  public $plural = 'CoPersonRoles';
  public $controller = 'co_person_roles';
  ...
  public function run() {
    ...
    $body["$this->plural"][0]['Affiliation'] = 'member';
    ...
    $request['method'] = 'GET';
    $request['uri']['path'] = "$this->controller.json";
    $request['uri']['query']['couid'] = 1;

    try {
      $result = $client->call($request, 200);
    } catch (Exception $e) {
      $msg = "$this->name View (per COU) exception:" . $e->getMessage();
      throw new Exception($msg);
    }
    ...
    if(!$foundIt) {
      $msg = "$this->name View (per COU) did not find created";
      throw new Exception($msg);
    }
    ...
    // Add with no Status should return 400
    $request = array();
    $request['uri']['path'] = "$this->controller.json";
    ...
    try {
      $result = $client->call($request, 400);
    } catch (Exception $e) {
      $msg = "$this->name Add exception:" . $e->getMessage();
      throw new Exception($msg);
    }
